...
<template>
  <AppLayout>
    <div class="max-w-4xl mx-auto space-y-8">
      ...
      <div class="bg-slate-800/60 border border-slate-700/60 rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xl">
        <div>
          <div class="flex items-center gap-3">
            <h1 class="text-2xl font-bold text-slate-100">Live Order Tracking</h1>
      ...
      <div v-if="order.status === 'completed' && isCustomerOwner" class="p-6 rounded-2xl bg-slate-800/80 border border-slate-700/80 shadow-2xl space-y-4">
        <!-- Existing Submitted Review View -->
        <div v-if="order.review" class="space-y-3">
          <div class="flex items-center justify-between border-b border-slate-700/60 pb-3">
            <div class="flex items-center gap-2">
              <span class="text-xl">⭐</span>
              <h3 class="text-sm font-bold text-slate-100">Your Submitted Experience Review</h3>
            </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div class="bg-slate-900/80 p-3.5 rounded-xl border border-slate-800 space-y-1">
              <span class="text-slate-400 block font-semibold text-[11px]">Laundry Shop Wash Quality:</span>
              ...
              <p v-if="order.review.shop_comment" class="text-slate-300 italic text-[11px]">"{{ order.review.shop_comment }}"</p>
            </div>

            <div v-if="order.rider" class="bg-slate-900/80 p-3.5 rounded-xl border border-slate-800 space-y-1">
              <span class="text-slate-400 block font-semibold text-[11px]">Rider Delivery Speed:</span>
              <div class="text-amber-400 font-bold text-base">
                {{ '★'.repeat(order.review.rider_rating) }}{{ '☆'.repeat(5 - order.review.rider_rating) }} ({{ order.review.rider_rating }}/5)
              </div>
              <p v-if="order.review.rider_comment" class="text-slate-300 italic text-[11px]">"{{ order.review.rider_comment }}"</p>
            </div>
          </div>
        </div>
        ...
        <form v-else @submit.prevent="submitReview" class="space-y-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-base font-bold text-slate-100">Rate Your Laundry & Delivery Experience</h3>
              <p class="text-xs text-slate-400">Help us maintain top washing quality and fast rider deliveries</p>
            </div>
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <!-- Shop Wash Rating -->
            <div class="bg-slate-900/90 p-4 rounded-xl border border-slate-700/80 space-y-2">
              <label class="block font-bold text-slate-200 uppercase text-[11px]">Laundry Shop Washing ({{ order.shop?.name }}) *</label>
              <div class="flex items-center gap-1.5 text-2xl">
                ...
                <span class="text-xs text-slate-400 font-bold ml-2">({{ reviewForm.shop_rating }}/5 Stars)</span>
              </div>
              <input
                v-model="reviewForm.shop_comment"
                type="text"
                placeholder="Comment on washing quality / scent (optional)"
                class="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-700 text-slate-100 text-xs focus:border-amber-500"
              />
            </div>

            <!-- Rider Delivery Rating -->
            <div v-if="order.rider" class="bg-slate-900/90 p-4 rounded-xl border border-slate-700/80 space-y-2">
              <label class="block font-bold text-slate-200 uppercase text-[11px]">Rider Delivery Speed ({{ order.rider?.first_name }}) *</label>
              <div class="flex items-center gap-1.5 text-2xl">
                <button
                  v-for="star in 5"
                  :key="'rider-' + star"
                ...
                <span class="text-xs text-slate-400 font-bold ml-2">({{ reviewForm.rider_rating }}/5 Stars)</span>
              </div>
              <input
      ...
      <div class="bg-slate-800/60 border border-slate-700/60 rounded-2xl p-8 shadow-xl space-y-6">
        <h2 class="text-base font-bold text-slate-100 uppercase tracking-wider">Garment Care Progress Stepper</h2>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div
            ...
            :class="[
              idx <= currentStepIndex ? 'bg-sky-500/10 border-sky-500 text-sky-300' : 'bg-slate-950/40 border-slate-800 text-slate-500 opacity-60'
            ]"
          >
      ...
      <div v-if="order.fulfillment_type === 'home_delivery'" class="bg-slate-800/60 border border-slate-700/60 rounded-2xl p-6 space-y-4 shadow-xl">
        <div class="flex items-center justify-between border-b border-slate-700 pb-4">
          <div>
            ...
            <p class="text-xs text-slate-400">Review price proposals from available riders and choose your preferred delivery partner</p>
          </div>
        ...
        <div v-if="order.rider" class="bg-slate-950 p-5 rounded-2xl border border-purple-500/30 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-lg">
          <div class="flex items-center gap-4">
            ...
            <div>
              <div class="flex items-center gap-2">
                <strong class="text-slate-100 text-sm block font-bold">{{ order.rider?.first_name }} {{ order.rider?.last_name }}</strong>
              ...
              <p class="text-slate-400 text-xs mt-0.5">
                Vehicle: <strong class="text-slate-200">{{ order.rider?.rider_profile?.vehicle_type || 'Motorcycle' }}</strong>
          ...
          <div class="text-right border-t sm:border-t-0 sm:border-l border-slate-800 pt-3 sm:pt-0 sm:pl-6 w-full sm:w-auto">
            <span class="text-[10px] text-slate-400 uppercase font-bold block">Agreed Delivery Fee:</span>
        ...
        <template v-else>
          <div v-if="!order.bids || order.bids.length === 0" class="bg-slate-950 p-6 rounded-xl border border-slate-800 text-center space-y-2 text-xs text-slate-400">
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div
              ...
              :key="bid.id"
              class="bg-slate-950 p-4 rounded-xl border border-slate-800 hover:border-purple-500/40 space-y-3 transition-all text-xs shadow-lg"
            >
              <div class="flex items-center justify-between border-b border-slate-800 pb-3">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <strong class="text-slate-200 block font-bold text-xs">{{ bid.rider?.first_name }} {{ bid.rider?.last_name }}</strong>
                    <span class="text-[10px] text-slate-400 capitalize">{{ bid.rider?.rider_profile?.vehicle_type || 'Delivery Rider' }}</span>
                  </div>
                </div>
              ...
              <p v-if="bid.note" class="text-slate-300 italic text-[11px]">"{{ bid.note }}"</p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <!-- Items List -->
        <div class="md:col-span-2 bg-slate-800/60 border border-slate-700/60 rounded-2xl p-6 space-y-4 shadow-xl">
          <h3 class="text-sm font-bold text-slate-200 uppercase tracking-wider">Garments & Treatment Services</h3>

          <div class="space-y-3">
            <div
              v-for="item in order.items"
              :key="item.id"
              class="p-4 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between text-xs"
            >
              <div>
                <span class="font-bold text-slate-100 block">{{ item.category_name }}</span>
                <span class="text-slate-400 block mt-0.5">{{ item.service_name }} — ₦{{ Number(item.unit_price).toLocaleString() }} each</span>
              </div>
              <div class="text-right font-mono">
                <span class="text-slate-400 block font-bold">Qty: {{ item.quantity }}</span>
        ...
        <div class="bg-slate-800/60 border border-slate-700/60 rounded-2xl p-6 space-y-6 shadow-xl text-xs">
          <!-- Shop Details -->
          <div class="space-y-2 border-b border-slate-700 pb-4">
            ...
            <p class="font-bold text-slate-100 text-sm">{{ order.shop?.name }}</p>
            <p class="text-slate-400">📍 {{ order.shop?.address }}</p>
            <p class="text-slate-400 font-mono">📞 {{ order.shop?.phone }}</p>
          </div>
          ...
          <div class="space-y-2 border-b border-slate-700 pb-4">
            ...
            <p class="text-slate-400">{{ order.delivery_address || 'Shop Location' }}</p>
          </div>
          ...
          <div class="space-y-2 font-mono">
            ...
            <div class="flex justify-between text-slate-100 font-bold text-sm">
      ...
      <div v-if="isSuperAdmin && order.status_logs && order.status_logs.length > 0" class="bg-slate-800/60 border border-purple-500/30 rounded-2xl p-6 space-y-4 shadow-xl">
        <div class="flex items-center justify-between border-b border-slate-700/60 pb-3">
          <div class="flex items-center gap-3">
            <span class="text-2xl">📜</span>
            <div>
              <h3 class="text-sm font-bold text-purple-300">Order Audit Trail & Conflict Resolution Log</h3>
              <p class="text-[11px] text-slate-400">Timestamped record of all state transitions visible to Super Admins</p>
            </div>
          </div>
        ...
        <div class="space-y-3">
          <div
            ...
            :key="log.id"
            class="p-4 rounded-xl bg-slate-900/80 border border-slate-700/60 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3"
          >
            <div class="space-y-1">
              <div class="flex items-center gap-2">
                <strong class="text-slate-100 font-bold text-xs">{{ log.user_name || 'System' }}</strong>
                <span class="px-2 py-0.5 rounded-full text-[10px] uppercase font-bold bg-slate-800 text-purple-400 border border-purple-500/20">{{ log.user_role || 'system' }}</span>
              </div>
              ...
              <p v-if="log.notes" class="text-slate-400 text-[11px] italic">"{{ log.notes }}"</p>
            </div>
            <span class="text-[10px] font-mono text-slate-400 shrink-0 bg-slate-950 px-2.5 py-1 rounded-lg border border-slate-800">{{ new Date(log.created_at).toLocaleString() }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
        <div class="relative bg-slate-900 border border-slate-700/80 rounded-3xl p-6 max-w-lg w-full shadow-2xl space-y-4 text-center overflow-hidden">
          <!-- Modal Header & Close Button -->
          <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            <div class="text-left">
              <h3 class="text-base font-bold text-slate-100">{{ selectedRiderImage.name }}</h3>
              <span class="text-xs text-purple-400 font-medium">{{ selectedRiderImage.vehicle }}</span>
            </div>
            <button
              @click="closeRiderImageModal"
              class="w-9 h-9 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white flex items-center justify-center font-bold text-base transition-colors"
            >
              ✕
            </button>
          </div>
          ...
          <div class="rounded-2xl overflow-hidden bg-slate-950 border border-slate-800 flex items-center justify-center max-h-[70vh] p-2">
            <img
