  ...
  <AppLayout>
    <div class="space-y-8">
      <div v-if="!shop" class="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-8 text-center space-y-4">
        ...
        <p class="text-xs text-gray-500 dark:text-slate-400 max-w-md mx-auto leading-relaxed">
      ...
      <template v-else>
        <!-- Shop Header Banner -->
        <div class="bg-white dark:bg-slate-900/90 border border-gray-200 dark:border-slate-800/80 rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xl">
          <div>
            <div class="flex items-center gap-3">
              <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-100">{{ shop.name }}</h1>
              <Badge :status="shop.status" />
            </div>
            <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">📍 {{ shop.address }} • 📞 {{ shop.phone }}</p>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <Link :href="`/shop/${shop.slug}`" target="_blank" class="px-4 py-2.5 rounded-xl text-xs font-semibold bg-gray-100 dark:bg-slate-800 hover:bg-slate-700 text-gray-800 dark:text-slate-200 border border-gray-200 dark:border-slate-700 transition-all">
              🔗 Storefront Preview
            </Link>
        ...
        <div
          ...
          class="bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800/80 border border-gray-200 dark:border-slate-700/60 rounded-2xl p-6 shadow-xl space-y-4"
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="font-bold text-gray-900 dark:text-slate-100 text-sm">Shop Settlement Account</h3>
                <p class="text-[11px] text-gray-500 dark:text-slate-400">Paystack Dedicated Virtual Account — customer payments go here directly</p>
              </div>
            </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <!-- Account Number -->
            <div class="bg-gray-50 dark:bg-slate-950/60 rounded-xl p-4 border border-gray-200 dark:border-slate-800/80 space-y-1.5">
              <p class="text-[10px] font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider">Account Number</p>
              <div class="flex items-center justify-between gap-2">
                <span class="font-mono font-black text-2xl text-gray-900 dark:text-slate-100 tracking-widest">{{ virtualAccount.account_number }}</span>
                <button
                  @click="copyToClipboard(virtualAccount.account_number, 'account')"
                  title="Copy account number"
                  class="p-1.5 rounded-lg transition-all"
                  :class="copiedField === 'account' ? 'text-emerald-400 bg-emerald-500/10' : 'text-gray-500 dark:text-slate-500 hover:text-sky-400 hover:bg-sky-500/10'"
                >
            ...
            <!-- Bank Name -->
            <div class="bg-gray-50 dark:bg-slate-950/60 rounded-xl p-4 border border-gray-200 dark:border-slate-800/80 space-y-1.5">
              <p class="text-[10px] font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider">Bank</p>
              <p class="font-bold text-gray-900 dark:text-slate-100 text-base">{{ virtualAccount.bank_name }}</p>
            </div>

            <!-- Account Name -->
            <div class="bg-gray-50 dark:bg-slate-950/60 rounded-xl p-4 border border-gray-200 dark:border-slate-800/80 space-y-1.5">
              <p class="text-[10px] font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider">Account Name</p>
              <p class="font-bold text-gray-900 dark:text-slate-100 text-base">{{ virtualAccount.account_name }}</p>
            </div>
          </div>

          <p class="text-[11px] text-gray-500 dark:text-slate-500 leading-relaxed pt-2 border-t border-gray-200 dark:border-slate-800/60">
        ...
        <div v-else-if="shop && shop.is_verified" class="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Your Paystack settlement account is being provisioned. Contact support if this persists.</p>
          </div>
        </div>
        ...
        >
          <div class="flex items-start gap-4">
            ...
            <div>
              ...
              <p class="text-xs mt-0.5" :class="isSubActive ? 'text-gray-500 dark:text-slate-400' : 'text-rose-400/70'">
          ...
          <Link
            ...
            :class="isSubActive ? 'bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-800 dark:text-slate-200 hover:border-sky-500/40 hover:text-sky-400' : 'bg-gradient-to-r from-sky-500 to-cyan-500 text-slate-950 shadow-sky-500/20'"
          >
        ...
        <div v-else-if="shop" class="bg-amber-500/5 border border-amber-500/30 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Subscribe to a monthly plan to unlock all operational features and start accepting customer orders.</p>
            </div>
          </div>
        ...
        <div v-if="stats.total_categories === 0 || stats.total_prices === 0" class="bg-gradient-to-r from-purple-900/40 via-slate-900 to-slate-900 border border-purple-500/30 rounded-2xl p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl">
          <div class="space-y-1">
            ...
            <p class="text-xs text-gray-700 dark:text-slate-300 max-w-xl leading-relaxed">
        ...
        <div v-if="shop_revenue_chart && category_breakdown_chart" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-2 bg-white dark:bg-slate-900/90 border border-gray-200 dark:border-slate-800/80 rounded-2xl p-6 shadow-xl space-y-4">
            <div class="flex items-center justify-between">
              <div>
                <h3 class="text-base font-bold text-gray-900 dark:text-slate-100">Shop Revenue Growth Trend</h3>
                <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Weekly revenue performance in NGN</p>
              </div>
          ...
          <div class="bg-white dark:bg-slate-900/90 border border-gray-200 dark:border-slate-800/80 rounded-2xl p-6 shadow-xl space-y-4">
            <div>
              <h3 class="text-base font-bold text-gray-900 dark:text-slate-100">Top Garment Categories</h3>
              <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Order volume breakdown by garment type</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Link href="/shop-admin/categories" class="p-5 rounded-2xl bg-white dark:bg-slate-900/60 border border-gray-200 dark:border-slate-800/80 hover:border-sky-500/40 transition-all group">
            <div class="text-2xl mb-2 group-hover:scale-110 transition-transform">🏷️</div>
            <h4 class="font-bold text-gray-800 dark:text-slate-200 text-sm group-hover:text-sky-400 transition-colors">1. Manage Categories</h4>
            <p class="text-xs text-gray-500 dark:text-slate-400 mt-1 leading-relaxed">Add item categories like Shirts, Jeans, Native wear, Duvets or clone from master templates.</p>
          </Link>

          <Link href="/shop-admin/services" class="p-5 rounded-2xl bg-white dark:bg-slate-900/60 border border-gray-200 dark:border-slate-800/80 hover:border-sky-500/40 transition-all group">
            <div class="text-2xl mb-2 group-hover:scale-110 transition-transform">🧺</div>
            <h4 class="font-bold text-gray-800 dark:text-slate-200 text-sm group-hover:text-sky-400 transition-colors">2. Define Services</h4>
            <p class="text-xs text-gray-500 dark:text-slate-400 mt-1 leading-relaxed">Create services like Wash Only, Wash & Iron, Starch & Press treatment options.</p>
          </Link>

          <Link href="/shop-admin/pricing" class="p-5 rounded-2xl bg-white dark:bg-slate-900/60 border border-gray-200 dark:border-slate-800/80 hover:border-sky-500/40 transition-all group">
            <div class="text-2xl mb-2 group-hover:scale-110 transition-transform">💳</div>
            <h4 class="font-bold text-gray-800 dark:text-slate-200 text-sm group-hover:text-sky-400 transition-colors">3. Set Price Matrix</h4>
            <p class="text-xs text-gray-500 dark:text-slate-400 mt-1 leading-relaxed">Assign custom prices for Category × Service pairs or import all master pricing.</p>
          </Link>
        </div>
      </template>
    </div>
