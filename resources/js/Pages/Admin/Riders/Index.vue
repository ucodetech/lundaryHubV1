  ...
  <AppLayout>
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-100">Rider Verification & KYC Audit</h1>
          <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">Review rider verification documents, live selfie face verification, and manage approval statuses</p>
        </div>
      </div>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-slate-700/40">
            <tr v-for="rider in riders.data" :key="rider.id" class="hover:bg-slate-800/40 transition-colors">
              <td class="py-4 px-6 font-semibold text-gray-800 dark:text-slate-200">
                ...
                <div class="text-xs text-gray-500 dark:text-slate-400 font-normal">{{ rider.user?.phone }}</div>
              </td>
              <td class="py-4 px-6 text-gray-700 dark:text-slate-300 text-xs capitalize">
            ...
            <tr v-if="!riders.data || riders.data.length === 0">
              <td colspan="4" class="py-12 text-center text-gray-500 dark:text-slate-400 text-xs">
      ...
      <div v-if="showAuditModal && selectedRider" class="fixed inset-0 bg-gray-50 dark:bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-6 w-full max-w-3xl space-y-6 shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar">
          <div class="flex items-center justify-between border-b border-gray-200 dark:border-slate-800 pb-4">
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">📞 {{ selectedRider.user?.phone }} • ✉️ {{ selectedRider.user?.email }}</p>
            </div>
            <button @click="closeModal" class="text-gray-500 dark:text-slate-400 hover:text-white text-lg">✕</button>
          </div>

          <!-- Rider Vehicle & License Info -->
          <div class="grid grid-cols-3 gap-4 bg-gray-50 dark:bg-slate-950/80 border border-gray-200 dark:border-slate-800 rounded-xl p-4 text-xs">
            <div>
              <span class="text-gray-500 dark:text-slate-500 block uppercase font-semibold text-[10px]">Vehicle Type</span>
              <span class="font-bold text-gray-800 dark:text-slate-200 capitalize">{{ selectedRider.vehicle_type }}</span>
            </div>
            <div>
              <span class="text-gray-500 dark:text-slate-500 block uppercase font-semibold text-[10px]">Plate Number</span>
              <span class="font-bold text-gray-800 dark:text-slate-200">{{ selectedRider.vehicle_plate || '—' }}</span>
            </div>
            <div>
              <span class="text-gray-500 dark:text-slate-500 block uppercase font-semibold text-[10px]">License Number</span>
              <span class="font-bold text-gray-800 dark:text-slate-200">{{ selectedRider.license_number || '—' }}</span>
            </div>
          </div>

          <!-- Submitted Verification Documents & Face Selfie -->
          <div class="space-y-3">
            <h4 class="text-xs font-bold text-gray-800 dark:text-slate-200 uppercase tracking-wider">Submitted Identity & Live Face Documents</h4>
            ...
            <div v-if="getKycDocs(selectedRider).length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              <div
                ...
                :key="doc.id"
                class="bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl p-3 space-y-2 flex flex-col justify-between"
              >
                <!-- Image or PDF Preview -->
                <div class="relative aspect-video rounded-lg overflow-hidden bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 flex items-center justify-center">
                  <img
                ...
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span class="text-[10px] font-mono text-gray-500 dark:text-slate-400">{{ doc.status }}</span>
                </div>
            ...
            <div v-else class="py-8 text-center bg-gray-50 dark:bg-slate-950/60 border border-gray-200 dark:border-slate-800 rounded-xl text-gray-500 dark:text-slate-400 text-xs">
              No KYC documents uploaded by this rider yet.
            </div>
          </div>

          <!-- Audit Modal Action Buttons -->
          <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-slate-800">
            <button
              type="button"
              @click="closeModal"
