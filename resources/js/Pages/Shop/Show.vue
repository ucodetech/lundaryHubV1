...
<template>
  ...
  <div class="min-h-screen bg-slate-950 text-slate-100 font-sans">
    <!-- Navbar Header -->
    <header class="h-16 border-b border-slate-800 px-6 flex items-center justify-between sticky top-0 bg-slate-950/90 backdrop-blur-md z-30">
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          ...
          class="relative px-4 py-2 rounded-xl text-xs font-bold bg-slate-900 border border-slate-700 hover:border-sky-500 text-sky-400 flex items-center gap-2 transition-all shadow"
        >
          <span>🛒 Cart</span>
    ...
    <div class="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div class="bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800 border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-6">
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div class="space-y-3">
            ...
            <div class="flex flex-wrap gap-4 text-xs text-slate-400 pt-1 font-mono">
        ...
        <div class="flex flex-wrap items-center gap-3 pt-2">
          <span
            ...
            :class="shop.offers_home_delivery ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-slate-800 text-slate-500 border-slate-700 line-through'"
          >
          ...
          <span
            class="px-3 py-1 rounded-xl text-xs font-bold border"
            :class="shop.offers_store_pickup ? 'bg-sky-500/10 text-sky-400 border-sky-500/20' : 'bg-slate-800 text-slate-500 border-slate-700 line-through'"
          >
      ...
      <div class="bg-slate-900 border border-slate-800 rounded-3xl p-8 space-y-6 shadow-2xl">
        <h2 class="text-xl font-bold text-slate-100">Select Garments & Add to Basket</h2>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label class="block text-xs font-semibold text-slate-400 uppercase mb-2">1. Select Garment Category</label>
            <select
          ...
          <div>
            <label class="block text-xs font-semibold text-slate-400 uppercase mb-2">2. Laundry Treatment</label>
            <select
          ...
          <div>
            ...
            <input
              ...
              type="number"
              min="1"
              class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 text-sm focus:border-sky-500 font-mono"
            />
          </div>
        </div>

        <div class="p-6 rounded-2xl bg-slate-950 border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-6">
            <div>
              <span class="text-xs text-slate-400 uppercase tracking-wider font-semibold">Unit Price</span>
              <h3 class="text-2xl font-bold text-slate-100 font-mono">
    ...
    <div v-if="showCartDrawer" class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex justify-end">
      <div class="w-full max-w-md bg-slate-900 border-l border-slate-800 h-full p-6 flex flex-col justify-between shadow-2xl space-y-6">
        <div class="space-y-6">
          <div class="flex items-center justify-between border-b border-slate-800 pb-4">
            <div class="flex items-center gap-2">
              <span class="text-xl">🧺</span>
              <h2 class="text-lg font-bold text-slate-100">Your Basket</h2>
            </div>
            <button @click="showCartDrawer = false" class="text-slate-400 hover:text-white text-xl">✕</button>
          </div>

          <div v-if="cart.length === 0" class="py-12 text-center text-slate-400 text-xs">
          ...
          <div v-else class="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
            ...
            >
              <div>
                <h4 class="font-bold text-sm text-slate-200">{{ item.category_name }}</h4>
                <p class="text-xs text-slate-400">{{ item.service_name }} • ₦{{ item.unit_price.toLocaleString() }} / item</p>
              </div>

              <div class="flex items-center gap-3">
                <div class="flex items-center gap-1 border border-slate-800 rounded-lg p-1 bg-slate-900">
                  <button @click="updateCartQuantity(idx, -1)" class="w-6 h-6 rounded bg-slate-800 text-xs font-bold text-slate-300 hover:bg-slate-700">-</button>
                  <span class="text-xs font-bold px-2 font-mono text-slate-100">{{ item.quantity }}</span>
                  <button @click="updateCartQuantity(idx, 1)" class="w-6 h-6 rounded bg-slate-800 text-xs font-bold text-slate-300 hover:bg-slate-700">+</button>
                </div>
        ...
        <div v-if="cart.length > 0" class="space-y-4 border-t border-slate-800 pt-4">
          <div class="flex justify-between items-center text-sm font-bold text-slate-200">
    ...
    <!-- Checkout Modal -->
    <div v-if="showCheckoutModal" class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div class="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-lg p-6 space-y-6 shadow-2xl">
        <div class="flex items-center justify-between border-b border-slate-800 pb-4">
          <h2 class="text-lg font-bold text-slate-100">🛒 Complete Booking & Order</h2>
          <button @click="showCheckoutModal = false" class="text-slate-400 hover:text-white text-xl">✕</button>
        </div>

        <form @submit.prevent="submitOrderBooking" class="space-y-4">
          <!-- Fulfillment Method Selector -->
          <div>
            <label class="block text-xs font-semibold text-slate-400 uppercase mb-2">Fulfillment Method *</label>
            <div class="grid grid-cols-2 gap-3">
              <button
                ...
                :class="checkoutForm.fulfillment_type === 'home_delivery' ? 'bg-sky-500/10 border-sky-500 text-sky-400 font-bold' : 'bg-slate-950 border-slate-800 text-slate-400'"
              >
                <div class="text-xs">🚚 Home Delivery</div>
                <div class="text-[10px] text-slate-400 mt-0.5">+ ₦{{ Number(shop.delivery_fee).toLocaleString() }}</div>
              </button>
              ...
              <button
                ...
                class="p-3.5 rounded-xl border text-left transition-all disabled:opacity-40"
                :class="checkoutForm.fulfillment_type === 'store_pickup' ? 'bg-sky-500/10 border-sky-500 text-sky-400 font-bold' : 'bg-slate-950 border-slate-800 text-slate-400'"
              >
                <div class="text-xs">🏬 Store Self Pickup</div>
                <div class="text-[10px] text-slate-400 mt-0.5">Free Pickup</div>
              </button>
            </div>
          </div>

          <div v-if="checkoutForm.fulfillment_type === 'home_delivery'">
            <label class="block text-xs font-semibold text-slate-400 uppercase mb-1">Doorstep Delivery Address *</label>
            <textarea
              v-model="checkoutForm.delivery_address"
              required
              rows="2"
              placeholder="Full address for garment pickup & delivery..."
              class="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 text-xs focus:border-sky-500"
            ></textarea>
          </div>

          <div>
            <label class="block text-xs font-semibold text-slate-400 uppercase mb-1">Special Garment Instructions / Notes</label>
            <textarea
          ...
          <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2 text-xs font-mono">
            ...
            <div class="flex justify-between text-slate-100 font-bold border-t border-slate-800 pt-2 text-sm">
