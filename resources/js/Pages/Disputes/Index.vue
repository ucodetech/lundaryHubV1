  ...
  <AppLayout>
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-slate-100">My Support & Dispute Tickets</h1>
          <p class="text-xs text-slate-400 mt-1">Track status and resolution progress for order issues</p>
        </div>

        <Link
          href="/orders"
          class="px-5 py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-slate-200 hover:text-white font-bold text-xs shadow transition-all"
        >
          📦 View My Orders
        </Link>
      </div>
      ...
      <div class="space-y-4">
        ...
        >
          <div class="space-y-2">
            ...
            <h3 class="font-bold text-slate-200 text-sm">{{ dispute.subject }}</h3>
          ...
          <Link
            ...
            class="px-5 py-2.5 rounded-xl bg-slate-900 border border-slate-700 hover:border-purple-500 text-xs font-bold text-purple-400 transition-all shadow shrink-0"
          >
            Open Ticket Thread →
          </Link>
        </div>

        <div v-if="!disputes.data || disputes.data.length === 0" class="bg-slate-800/40 border border-slate-700/60 rounded-2xl p-12 text-center text-slate-400 text-xs shadow-xl space-y-2">
          <span class="text-4xl block">🛡️</span>
          <h3 class="font-bold text-slate-200 text-sm">No Active Dispute Tickets</h3>
