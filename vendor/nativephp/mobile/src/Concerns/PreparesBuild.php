...
trait PreparesBuild
{
    ...
    protected function prepareLaravelBundle(bool $excludeDevDependencies = true): void
    {
        ...
        try {
            ...
            $this->logToFile('  Copying vendor directory...');
            $this->components->task('Copying vendor directory', function () use ($source, $tempDir) {
                if (is_dir("{$source}/vendor")) {
                    File::copyDirectory("{$source}/vendor", "{$tempDir}/vendor");
                }
                return true;
            });
    ...
    {
        // Using PHP native ZipArchive for cross-platform reliability
